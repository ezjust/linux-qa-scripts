...
echo $partnumbers is partnumbers
...
function lvm {
	...
	vgcreate lvm1 $device1 $device2
	lvcreate -l 100%FREE -n lvm1 lvm1
	mkfs.ext4 /dev/lvm1/lvm1
	mkdir /mnt/lvm1
	mount /dev/lvm1/lvm1 /mnt/lvm1
	echo "/dev/lvm1lvm1" "/mnt/lvm1/lvm1" "ext4" "defaults" "0" "2" >> /etc/fstab
}

function raid {
	mdadm --zero-superblock $device5 /dev/sdb6
	mdadm --create /dev/md0 --level=1 --raid-devices=2 $device5 $device6
	mkfs.ext3 /dev/md0
	mkdir /raid1
	mount /dev/md0 /raid1
	echo "/dev/md0" "/raid1" "ext3" "defaults" "0" "2" >> /etc/fstab
}
