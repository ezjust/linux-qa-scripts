...
disk1_size=$(fdisk -l "$disk1" | grep Disk | awk '{print $3}' | head -1) >> $ignore
disk1_=$(echo $disk1 | cut -d"/" -f3)
disk1_sectors=$(cat /sys/block/$disk1_/size)  >> $ignore
disk1_partition_sectors="$(echo $disk1_size / 7 | bc)" >> $ignore
disk2=${disks[1]} >> $ignore
disk2_size=$(fdisk -l $disk2 | grep Disk | awk '{print $3}' | head -1) >> $ignore
disk2_=$(echo $disk2 | cut -d"/" -f3)
disk2_sectors=$(cat /sys/block/$disk2_/size)  >> $ignore
disk2_partition_sectors=$(echo $disk2_size / 7 | bc) >> $ignore
disk3=${disks[2]} >> $ignore
disk3_size=$(fdisk -l $disk3 | grep Disk | awk '{print $3}' | head -1) >> $ignore
disk3_=$(echo $disk3 | cut -d"/" -f3)
disk3_sectors=$(cat /sys/block/$disk3_/size)  >> $ignore
disk3_partition_sectors=$(echo $disk3_size / 7 | bc) >> $ignore
disk4=${disks[3]} >> $ignore
disk4_size=$(fdisk -l $disk4 | grep Disk | awk '{print $3}' | head -1) >> $ignore
disk4_=$(echo $disk4 | cut -d"/" -f3)
disk4_sectors=$(cat /sys/block/$disk4_/size)  >> $ignore
disk4_partition_sectors=$(echo $disk4_size / 7 | bc) >> $ignore
disk5=${disks[4]} >> $ignore
disk5_size=$(fdisk -l $disk5 | grep Disk | awk '{print $3}' | head -1) >> $ignore
disk5_=$(echo $disk5 | cut -d"/" -f3)
disk5_sectors=$(cat /sys/block/$disk5_/size)  >> $ignore
disk5_partition_sectors=$(echo $disk5_size / 7 | bc ) >> $ignore
...
if [[ "$disk1_size" < "10.7" || "$disk2_size" < "10.7" || "$disk3_size" < "10.7" || "$disk4_size" < "10.7" || "$disk5_size" < "10.7" ]]; then
	echo "Not all disks have needed size : 10.7G"
	echo ""
...
function disk_primary_partitions_create {
	...
	do
		...
		do 
			...
			(echo n; echo p; echo $i; echo ; echo "+""${disk_partition_sectors[$m]}"G; echo w) | fdisk ${disk[$m]} >> /dev/null 2>&1
			sleep 0.2
		...
		for i in {5..7}
		do
			(echo n; echo ; echo "+"${disk_partition_sectors[$m]}G; echo w) | fdisk ${disk[$m]} >> /dev/null 2>&1
			sleep 0.5
		done
